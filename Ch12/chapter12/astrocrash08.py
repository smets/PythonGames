        ...
        if self.overlapping_sprites:
            for sprite in self.overlapping_sprites:
                if isinstance(sprite, Asteroid):
                    sprite.die()
                    self.die()
                if isinstance(sprite, PowerUp) and isinstance(self, Ship):
                    self.powerUp()
                    sprite.die()
    ...
    def die(self):
        ...
        if self.hits == 0:
            ...
            if self.size != Asteroid.SMALL:
                #CHANGE: Now large asteroids spawn 3 mediums, mediums spawn 2
                #smalls when hit, some asteroids take 2 hits
...
class Ship(Collider):
    ...
    def powerUp(self):
        self.PowerOn = True
        self.powerTime = Ship.POWERUP_DUR
    def update(self):
        ...
        if games.keyboard.is_pressed(games.K_SPACE) and self.missile_wait == 0:
            ...
            if self.PowerOn:
                it = 4
            for i in range(it):
                new_missile = Missile(self.x, self.y, self.angle+((360/4)*(i)))
                games.screen.add(new_missile)        
                self.missile_wait = Ship.MISSILE_DELAY
        #CHANGE check to see if we need to reduce the counter for powerup duration         
        if self.powerTime > 0:
            self.powerTime -= 1
        if self.powerTime == 0:
            self.PowerOn = False;
        #CHANGE add a new powerup to the board randomly
        if random.randrange(0, PowerUp.CHANCES) == 0:
            new_powerup = PowerUp(game = self.game,
                                  x = random.choice([1,-1]),
                                  y = random.choice([1,-1]))
            games.screen.add(new_powerup)
...
#CHANGE Added Powerup Class
class PowerUp(Wrapper):
    IMAGE = games.load_image("mega_mushroom.png");
    SPEED = 1
    CHANCES = 2000
    FREQ_UPDATE = 25
    def __init__(self, game, x, y):
        super(PowerUp, self).__init__(
            image = PowerUp.IMAGE,
            x = x, y = y,
            dx = random.choice([1, -1]) * PowerUp.SPEED, 
            dy = random.choice([1, -1]) * PowerUp.SPEED)
